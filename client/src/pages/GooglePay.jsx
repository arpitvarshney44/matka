...
const GooglePay = () => {
  ...
  useEffect(() => {
    loadGooglePayDetails()
  }, [])

  const loadGooglePayDetails = async () => {
    try {
      ...
      if (response.data.paymentDetails?.googlePay) {
        setMobileNumber(response.data.paymentDetails.googlePay)
      }
    } catch (error) {
      console.error('Error loading Google Pay details:', error)
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Google Pay number updated successfully!')
      navigate('/dashboard')
    } catch (error) {
      console.error('Error updating Google Pay number:', error)
      toast.error('Failed to update Google Pay number')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Google Pay details...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-green-800 shadow-lg px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold text-white">GooglePay</h1>
          </div>
        </div>
      </div>
