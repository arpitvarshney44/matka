...
const Paytm = () => {
  ...
  const loadPaytmDetails = async () => {
    try {
      ...
      if (response.data.paymentDetails?.paytm) {
        setMobileNumber(response.data.paymentDetails.paytm)
      }
    } catch (error) {
      console.error('Error loading Paytm details:', error)
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error('Failed to update Paytm number')
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading Paytm details...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="bg-green-800 shadow-lg px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold text-white">Paytm</h1>
          </div>
        </div>
      </div>
