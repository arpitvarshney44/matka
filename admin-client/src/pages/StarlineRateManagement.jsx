...
const StarlineRateManagement = () => {
  ...
  const fetchRates = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/starline/gamerates/admin`, {
  ...
  const handleSave = async () => {
    try {
      await axios.put(`${API_URL}/starline/gamerates/admin`, rates, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      toast.success('Starline rates updated!');
      setEditing(false);
      fetchRates();
    } catch (e) {
      toast.error('Failed to update starline rates');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="bg-blue-700 text-white px-4 py-3 rounded-t-lg flex items-center">
        ...
        <span className="font-bold text-lg">Starline Game Rates</span>
      </div>
      <div className="bg-white rounded-b-lg shadow-md p-6">
        <h2 className="text-2xl font-bold text-blue-700 mb-6 text-center">Add Starline Games Rate</h2>
...
export default StarlineRateManagement;